...
function TasksPage() {
	...
	const [taskFullCache, setTaskFullCache] = useState<Map<string, TaskFull>>(
		new Map(),
	);
	...
	const loadActiveExecutions = useCallback(async () => {
		try {
			const executions = await safeInvoke<TaskExecution[]>(
				"get_all_active_executions",
			);
	...
	const loadTaskFull = useCallback(
		async (taskId: string, projectPathArg: string) => {
			const full = await safeInvoke<TaskFull>("get_task_full", {
	...
	useEffect(() => {
		...
		safeListen<TaskUpdatedPayload>("task-updated", async (event) => {
			if (event.payload.source === "ai") {
				console.log(
					"[WorkoPilot] Task updated by AI, reloading and syncing...",
				);
				...
				{pendingTasks.length > 0 && (
					<div className="space-y-1">
						{pendingTasks.map((task) => {
							...
							const statusLabel = getTaskStatusLabel(taskFull || null);
							const highlight = getTaskStatusHighlight(taskFull || null);
							const isAICurrentlyWorking = isTaskAIWorking(taskFull || null);
							...
							return (
								<div
									key={task.id}
									className={`bg-[#232323] hover:bg-[#2a2a2a] transition-colors group ${highlightClasses} ${isExecuting ? "ring-1 ring-[#909d63]" : ""}`}
									style={{
										...(highlight === "border-l"
											? { borderLeftColor: statusColor }
											: {}),
										...(highlight === "ring" && !isExecuting
								...
								>
									...
									>
										{isExecuting || isAICurrentlyWorking ? (
											<svg
										...
										<span className="flex-1 text-[#d6d6d6] text-sm flex items-center gap-2">
											...
											{isExecuting && activeExec && (
												<span className="text-xs text-[#909d63] opacity-75">
													(Passo {activeExec.current_step}/
													{activeExec.total_steps})
												</span>
											)}
										</span>
										...
										<span
											className={`px-2 py-0.5 text-xs text-[#1c1c1c] ${getPriorityClass(task.priority)}`}
										>
											P{task.priority}
										</span>
										...
										{taskSubtasks.length > 0 &&
											taskSubtasks.every((s) => s.status === "done") && (
												<button
													onClick={(e) => {
														e.stopPropagation();
														reviewTask(task);
										...
										<div className="pl-8 pr-3 pb-2 space-y-1">
											{taskSubtasks.map((subtask) => (
												...
												>
													<button
														onClick={() =>
															toggleSubtaskInList(task.id, subtask.id)
														}
