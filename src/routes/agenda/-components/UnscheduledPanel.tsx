...
export const UnscheduledPanel = forwardRef<
	UnscheduledPanelRef,
...
>(function UnscheduledPanel({ projects, onTaskScheduled }, ref) {
	...
	const resetDistributionState = useAgendaStore(
		(s) => s.resetDistributionState,
	);
	...
	} = trpc.tasks.listUnscheduled.useQuery(
		filterProject && filterProject !== "__all__"
	...
	const tasks = rawTasks.filter((t) => {
		...
		if (
			...
			t.priority !== parseInt(filterPriority, 10)
		)
			return false;
		return true;
	});
				...
				{!isDistributionMode && (
					<div className="flex flex-col gap-2">
						<CustomSelect
							...
							onValueChange={(id) =>
								setFilterProject(id === "__all__" ? "" : id)
							}
							...
							renderTrigger={() => {
								const selected = projectItems.find(
									(p) => p.id === (filterProject || "__all__"),
								);
								return (
									<>
										{selected?.color ? (
						...
						<div className="flex gap-2">
							<CustomSelect
								...
								onValueChange={(id) =>
									setFilterCategory(id === "__all__" ? "" : id)
								}
							...
							<CustomSelect
								...
								onValueChange={(id) =>
									setFilterPriority(id === "__all__" ? "" : id)
								}
								triggerClassName="flex items-center gap-2 px-2 py-1.5 h-8 flex-1 border border-border bg-card rounded-md hover:bg-popover hover:border-muted-foreground transition-colors"
								contentClassName="min-w-[120px]"
								renderTrigger={() => {
									const selected = PRIORITIES.find(
										(p) => p.id === (filterPriority || "__all__"),
									);
									return (
										<>
											{selected?.color && (
