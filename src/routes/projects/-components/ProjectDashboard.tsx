...
export function ProjectDashboard({
	...
	if (
		...
		projectConfig.id !== localProjectConfig?.id
	) {
	...
	async function saveProjectName() {
		if (!selectedProjectId || !localProjectConfig || !editedProjectName.trim())
			return;
		try {
			await updateProjectMutation.mutateAsync({
				id: selectedProjectId,
			...
			setLocalProjectConfig({
				...localProjectConfig,
				name: editedProjectName.trim(),
			});
			const updated = projectsList.map((p) =>
				p.id === selectedProjectId
					? { ...p, name: editedProjectName.trim() }
					: p,
			);
	...
	return (
		<div className="space-y-4">
			...
			{localProjectConfig.business_rules &&
				localProjectConfig.business_rules.trim() && (
					<Card>
					...
					<CardContent className="space-y-2 pt-0">
						{activeTasks.map((task) => {
							...
							const containerClass =
								getTaskProgressStateContainerClass(taskFull);
							...
							return (
								...
								>
									{indicator === "spinner" && (
										<Loader2
											className="size-4 animate-spin shrink-0 text-chart-4"
									...
									<Badge variant={badgeVariant}>{progressLabel}</Badge>
					...
					{isTmuxConfigured ? (
						<div className="flex items-center gap-3">
							...
							<span className="text-xs text-muted-foreground">
								{localProjectConfig.routes.length} rota(s),{" "}
								{localProjectConfig.tmux_config.tabs.length} tab(s)
							</span>
						</div>
					) : (
						<div className="flex items-center justify-between">
