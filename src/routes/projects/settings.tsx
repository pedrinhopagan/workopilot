...
import { isTauri, safeInvoke } from "../../services/tauri";
import { trpc } from "../../services/trpc";
import { useDialogStateStore } from "../../stores/dialogState";
import { useSelectedProjectStore } from "../../stores/selectedProject";
import type { ProjectRoute, ProjectWithConfig, TmuxTab } from "../../types";
		...
		>
			...
			>
				<div className="flex items-center gap-3">
					<div
						className={cn("p-2 bg-background/60", danger && "text-destructive")}
						style={
							accentColor
		...
		<div className="space-y-4">
			...
			<div className="flex items-center gap-3">
				...
				{saved ? (
					<Badge
						variant="outline"
						className="gap-1.5 text-accent border-accent/40"
					>
						<Check size={12} />
						Salvo
					</Badge>
...
function SettingsPage() {
	...
	const [localConfig, setLocalConfig] = useState<ProjectWithConfig | null>(
		null,
	);
	const projectConfig =
		localConfig ?? (fetchedConfig as ProjectWithConfig | null) ?? null;
	...
	const [draggingRouteIndex, setDraggingRouteIndex] = useState<number | null>(
		null,
	);
	...
	async function createTmuxTabForRoute(
		route: ProjectRoute,
		config: ProjectWithConfig,
	) {
	...
	async function removeRoute(routeId: string) {
		...
		const newTabs = projectConfig.tmux_config.tabs.filter(
	...
	async function addTmuxTab() {
		...
		const usedRouteIds = new Set(
			projectConfig.tmux_config.tabs.map((t) => t.route_id),
		);
		const availableRoute = projectConfig.routes.find(
			(r) => !usedRouteIds.has(r.id),
		);
	...
	function isRootTab(tab: TmuxTab) {
		...
		return rootRoute
			? tab.route_id === rootRoute.id && tab.name === "oc"
			: false;
	}
	...
	async function handleRoutePathChange(route: ProjectRoute) {
		...
		const existingTab = projectConfig.tmux_config.tabs.find(
			(t) => t.route_id === route.id,
		);
		if (existingTab) {
	...
	function getRouteOptions() {
		if (!projectConfig) return [];
		return projectConfig.routes.map((r) => ({
			id: r.id,
			name: getRouteNameFromPath(r.path),
		}));
	}
	...
	function getSortedTabs() {
		if (!projectConfig) return [];
		return [...projectConfig.tmux_config.tabs].sort(
			(a, b) => a.order - b.order,
		);
	}
	...
	return (
		<>
			...
			<div
				className="shrink-0 border-b border-border animate-fade-in"
				style={{
								...
								return (
									...
									>
										...
										<input
											...
											value={route.path}
											onChange={(e) =>
												updateRoutePath(route.id, e.target.value)
											}
										...
										{route.env_path && (
											<Button
												...
												size="sm"
												onClick={() =>
													route.env_path && openEnvFile(route.env_path)
												}
												className="text-xs"
											>
												.env
								...
								return (
									...
									>
										...
										<CustomSelect
											...
											onValueChange={(value) =>
												handleTabRouteChange(tab, value)
											}
										/>
					...
					>
						<div className="flex items-center justify-between">
							<div>
								...
								<p className="text-xs text-muted-foreground mt-0.5">
									Ao excluir, todas as tarefas associadas também serão
									removidas.
								</p>
							</div>
							<Button
